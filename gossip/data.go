...
type notifyMap struct {
	data map[common.GossipType][]*common.RegisteredModule
	sync.RWMutex
}

func NewNotifyMap() *notifyMap {
	return &notifyMap{
		data: make(map[common.GossipType]([]*common.RegisteredModule)),
	}
}

func (nm *notifyMap) Load(gossip_type common.GossipType) []*common.RegisteredModule {
	nm.RLock()
	defer nm.RUnlock()
...
func (nm *notifyMap) AddChannelToType(gossip_type common.GossipType, new_channel *common.RegisteredModule) {
	nm.Lock()
	defer nm.Unlock()
