...
In this last scenario, a Gossip Announce message is sent to the Vertical API (vertAPI) thread. The message goes to the Main (main) module which will verify that the type is registered and in case proceed by forwarding the message to the Strategy (strat) thread. The strategy will then send the message to the Horizontal API (hzAPI) which will take care of actually sending the message to other peers.
...
In this scenario, we depict the handling of a peer \textit{PUSH} message \ref{fig:msg-relay}. First, the message is received by the Horizontal API (hzAPI), then it goes to the concrete Strategy (strat). The strategy performs will process the message accordingly and ask main (main) to perform a gossip notification for such message. If such Gossip Type is registered, Main will then instruct Vertical API (vertAPI) to send to connected modules such message. After that Vertical have done so, it will receive a Gossip Validation message. The message will travel the threads back to the strategy, which if the message is valid will begin to spread it itself.
...
This will mitigate a Sybil Attack, since the attacker, in order to create multiple identities, has to overcome multiple Proofs of Work. But the attacker can still share its resources by sequentially add nodes. To counteract this possibility, the idea is to request PoW periodically. More on this can be found in the Future Work section \ref{sec:future}.

\subsubsection{Periodical Proofs of Work}

A node will periodically ask to the accepted peers for a PoW. At the same time the node will be prompted by the peer which it has connected to for PoWs. An attacker would have to compute such PoWs for each fake identity, making resource sharing not possible anymore.

Currently the idea needs to be further explored, but multiple requests for PoW could be aggregated into a single PoW so that a node does not waste to much resources into multiple PoW. Still an attacker would need to compute 1 PoW for each fake identity. PoWs could also be preemptively asked to facilitate even more this aggregation (since there is no global synchronization). More on this matter can be found in 
