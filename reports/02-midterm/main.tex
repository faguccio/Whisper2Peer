...
\begin{itemize}
	\item both modules work fairly similar. Both
		\begin{itemize}
			\item provide a layer of abstraction on top of the raw tcp sockets
			\item use two threads, one for sending and one for writing, per connection + one general thread to establish (and dispatch new connections)
				for reading this is strictly needed because we need to listen to multiple tcp connections (basically an (e)poll)
				for writing this may not be strictly necessary (just spwan a goroutine which sends this specific message) but it seems more uniform if reading and writing are symmetric

				Also this from the perspective of the main logic this is somewhat similar to asyncIO and the write is non blocking (under the assumption the channel was empty before sending to it)

				Also multiple clients are possible

				the use of channels made the communication with the threads fairly simple, also they implicitly synchronize the sending of messages (not needed though as concurrent write on the tcp socket in go) and synchronize the receiving thread and the reading thread
			\item communication with these threads happens through go channels
		\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item for now only one message concrete type needed push
	\item push is (obv) needed to spread information in the network
	\item p2p protocol is specified/implemented using capnproto (see -- ref back to structure)
		=> concrete message format not specified, only which fields are available and what type they have (see table)
	\item Actual sent messages are of type message which only contains a single union over all messages possibly being sent over this connection. At the moment push message type is the only one contained in this union. This way er are ready when more types are needed
\end{itemize}
% TODO(Lukas)
