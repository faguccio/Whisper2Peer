...
In this document we will introduce our module \textbf{Whisper2Peer} overall design. We will first go through the changes to the initial assumptions, mainly regarding Testing and Quality of Software. Then we will show the module architecture, both from a logical and a concurrent standpoint. We will give sample scenarios to understand how messages are processed and travel the program. We will show also our own abstractions over the networking.

Later we will discuss the Security Measure we intend to implement, and also the peer to peer protocol we put in place. Finally we will explore the work it remains to be done and how the workload was distributed between the members.
...
On the other hand we explored the tool \textit{go-sumtype} \ref{sumtypes} which allows us compile time checking for sum types, which is not included in the language out of the box. 
...
Those modules will create threads which can communicate with message passing, as can be seen again in \cref{fig:overview}. This design decision allows the program to be highly scalable due to the degree of parallelization.
...
\texttt{Main} is the entry point of the whole module. This thread will spawn a Listener \texttt{Vertical API} thread that will keep accepting new connection to the module. For each new connection, two new routines are spawned: one that will receive and process messages and one that will send messages. Communication directed to or from routines created by \texttt{Vertical API} happen respectively with the \texttt{toVert} and \texttt{fromVert} channels.

Then, \texttt{Main} will create the \texttt{Strategy} thread, which will then spawn a Listener thread for the \texttt{Horizontal API}. The latter, similarly to the \texttt{Vertical API}, will accept new connection and create for each one two threads, one for reading and one for writing. Communication between \texttt{Main} and \texttt{Strategy} routines happen in the \texttt{fromStrat} and \texttt{toStrat} channels, while \texttt{Strategy} communicates with threads of \texttt{Horizontal API} with the \texttt{fromHz} and \texttt{toHz} channels.

As already mentioned, there is actually no communication between \texttt{Horizontal API} goroutines and the \texttt{Main}. The communication can again be seen in \cref{fig:overview}. It is important to note that there will actually be multiple channels directed to the \textit{Vertical API} (\texttt{toVert}), as well as multiple channels directed to the \textit{Horizontal API} (\texttt{toHz}). This is because incoming messages from the network are aggregated on the same channel while outgoing messages will be sent to specific hosts, so different channels.
...
In this case the message passing is trivial (see \cref{fig:msg-notify}). The Vertical API (\texttt{vertAPI}) receive the \textit{Gossip Notifiy} message (\texttt{notify}), marshal it and send it to the main (\texttt{main}) thread where the Gossip Type will be registered. It's interesting to notice that what we store is the writing communication channel. As previously mentioned in \cref{process-creation}, there are multiple writing channels, one for each hosts. This will greatly simplify the sending procedure.
...
In this scenario, a Gossip Announce message is sent to the Vertical API (\texttt{vertAPI}) thread. The message (\texttt{ann}) goes to the Main (\texttt{main}) module which will verify that the type is registered and in case proceed by forwarding the message to the Strategy (\texttt{strat}) thread. The strategy will then send the message to the Horizontal API (\texttt{hzAPI}) which will take care of actually sending the message to other peers. In the form of a PUSH message. 

The delay in the Strategy, from reception to sending, is due to the implementation. Messages are sent in rounds, so they will wait until a new sending round starts.
...
In this last scenario, we depict the handling of a peer \textit{PUSH} message \ref{fig:msg-relay}. First, the message is received by the Horizontal API (hzAPI), then it goes to the concrete Strategy (strat). The strategy will process the message accordingly and ask main (main) to perform a gossip notification for such message (\texttt{notific}). If such Gossip Type is registered, Main will then instruct Vertical API (vertAPI) to send to connected modules such message (\texttt{notific}). After that Vertical have done so, it will wait to receive a Gossip Validation message (\texttt{val}). On reception, the information will travel the threads back to the strategy, which if the message is valid will begin to spread it itself. Otherwise it will be deleted from memory.
...
Currently, as a node starts, it will receive a list of the peer node to connect to. A peer will accept any new connection, which makes the system vulnerable to \textit{Sybils Attacks} and consequently \textit{Eclipse Attacks}. We plan to mitigate this problem by introducing a Identity Validation system based on a computational Proof of Work (PoW).

\subsection{Identity Validation}

When a peer is asked to accept a new connection, it will send a challenge (random nonce) $r$. Based on $r$ and a variable local part $v$, the other peer will compute a hash which should start with $n$ leading zeros, much like in the registration challenge for the project. $n$ can be a configuration parameters, meaning that nodes could choose how strong they want proof of work from newly connected nodes (and how much strong PoWs are they willing to compute when trying to connect to a peer).

This will mitigate a Sybil Attack, since the attacker, in order to create multiple identities, has to overcome multiple Proofs of Work. But the attacker can still share its resources by sequentially add nodes. To counteract this possibility, the idea is to request PoW periodically. 

\subsection{Periodical Proofs of Work}

A node will periodically ask to the neighboring peers for a PoW. At the same time the node will be prompted by each of its neighbors for PoWs. An attacker would have to compute such PoWs for each fake identity, making resource sharing not possible anymore.

Currently the idea needs to be further explored, but multiple requests for PoW could be aggregated into a single PoW so that a node does not waste to much resources into multiple PoW. Still an attacker would need to compute 1 PoW for each fake identity. PoWs could also be preemptively asked to facilitate even more this aggregation (since there is no global synchronization) and also mitigate DoS of attacks. PoW requests will also have a specific upper bound for the frequency.
...
Fabio focused on the developing the Strategy package, which was greatly written in peer programming, especially in the beginning when defining the whole interface. He also wrote the tests for the verticalAPI.
